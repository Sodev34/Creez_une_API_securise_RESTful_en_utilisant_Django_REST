...
class ProjectsPermissions(permissions.BasePermission):
    def has_permission(self, request, view):
        if request.method in permissions.SAFE_METHODS:
            return request.user.is_authenticated and Projects.objects.filter(contributor=request.user, id=view.kwargs.get('project_id')).exists()
        else:
...
class ContributorsPermissions(permissions.BasePermission):
    def has_permission(self, request, view):
        ...
        if request.method in permissions.SAFE_METHODS:
            return request.user.is_authenticated and Projects.objects.filter(contributor=request.user, id=project_id).exists()
        else:
            return request.user.is_authenticated and Projects.objects.filter(author=request.user, id=project_id).exists()
...
class IssuesPermissions(permissions.BasePermission):
    def has_permission(self, request, view):
        try:
            ...
            return request.user.is_authenticated and project in Projects.objects.filter(contributor=request.user)
...
class CommentsPermissions(permissions.BasePermission):
    def has_permission(self, request, view):
        ...
        try:
            ...
            if request.method in permissions.SAFE_METHODS:
                return Projects.objects.filter(contributor=request.user, id=project_id, issues__comments=comment_id).exists()
